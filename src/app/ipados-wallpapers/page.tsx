...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const canonicalUrl = withLanguageUrl(`${SITE_URL}/ipados-wallpapers`, language);
  const { title, description } = getCategorySeoCopy(language, 'ipados-wallpapers');
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates(`${SITE_URL}/ipados-wallpapers`),
    },
    openGraph: {
      title: fullTitle,
      description,
...
// iPadOS 落地页：展示 iPadOS 系统壁纸系列入口卡片。
export default function IPadOSWallpapersPage() {
  const cards = sortByDateDesc(iPadOSData).map((item) => ({
    name: item.name,
    date: item.date,
  ...
  return (
    <SeoLandingPage
      breadcrumbLabel="iPadOS Wallpapers"
      categoryKey="ipados-wallpapers"
