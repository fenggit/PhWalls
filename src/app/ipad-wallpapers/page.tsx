import type { Metadata } from 'next';

// Cloudflare Pages 部署必需，请勿删除
export const runtime = 'edge';

import iPadData from '@/data/iPad.json';
import { SITE_URL, getCategorySeoCopy } from '@/lib/seo';
import SeoLandingPage from '@/components/SeoLandingPage';
import { buildLanguageAlternates, getOpenGraphLocaleForLanguage, withLanguageUrl } from '@/lib/language';
import { sortByDateDesc } from '@/lib/data';
import { resolveMetadataLanguage } from '@/lib/metadata';

export async function generateMetadata(): Promise<Metadata> {
  const language = await resolveMetadataLanguage();
  const canonicalUrl = withLanguageUrl(`${SITE_URL}/ipad-wallpapers`, language);
  const { title, description } = getCategorySeoCopy(language, 'ipad-wallpapers');
  const fullTitle = `${title} | PhWalls`;

  return {
    title: fullTitle,
    description,
    alternates: {
      canonical: canonicalUrl,
      languages: buildLanguageAlternates(`${SITE_URL}/ipad-wallpapers`),
    },
    openGraph: {
      title: fullTitle,
      description,
      url: canonicalUrl,
      type: 'website',
      locale: getOpenGraphLocaleForLanguage(language),
    },
    twitter: {
      card: 'summary_large_image',
      title: fullTitle,
      description,
    },
  };
}

// iPad 落地页：展示 iPad 壁纸系列入口卡片。
export default function IPadWallpapersPage() {
  const cards = sortByDateDesc(iPadData).map((device) => ({
    name: device.name,
    date: device.date,
    count: device.item?.length || 0,
    imageKey: device.item?.[0]?.compressPath || device.item?.[0]?.originPath || null,
    wallpapers: device.item || [],
  }));

  return (
    <SeoLandingPage
      breadcrumbLabel="iPad Wallpapers"
      categoryKey="ipad-wallpapers"
      cardAspect="aspect-[4/3]"
      gridClass="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
      cards={cards}
    />
  );
}
